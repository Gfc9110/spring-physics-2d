...
export class Segment {
  ...
  projection(v: Vector) {
    ...
    return {
      projection: this.a.copy().add(ab.scale(t)),
      t,
      cos,
      acos: Math.acos(cos),
      inside,
...
export class Point {
  ...
  constructor(
    ...
    public position: Vector /*, private world: World*/,
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    if (this.structure.world.draggingPoint == this) {
      ...
      ctx.lineTo(
        this.structure.world.mousePosition.x,
        this.structure.world.mousePosition.y
      );
      ctx.stroke();
    }
  }
  ...
  update(delta: number, base?: number) {
    ...
    let collisions: Spring[] = this.structure.world.structures
      .filter(
        (st) =>
          st != this.structure &&
          st.boundingBox.intersects(
            this.structure.boundingBox
          ) /* && st.isPositionInside(newPosition)*/
      )
      .map((s) => s.shapes)
      .flat()
      .filter((sh) => sh.isPositionInside(newPosition))
      .map((st) =>
        this.closestSpring(
          st.springs.filter((s) => s.isSide),
          newPosition,
          true
        )
      );
    ...
    collisions.forEach((is) => {
      ...
      const averageFriction =
        (this.friction +
          (is.pointA.friction * (1 - t) + is.pointB.friction * t)) /
        2;

      const springVelocityAtImpact = is.pointA.velocity
        .copy()
        .scale(1 - t)
        .add(is.pointB.velocity.copy().scale(t));
      const springMassAtImpact =
        (is.pointA.mass * (1 - t) + is.pointB.mass * t) * 2;

      const totalMass = this.mass + springMassAtImpact;

      const springTangentVelocityAtImpact = springVelocityAtImpact.projectOn(
        is.pointA.position
          .copy()
          .sub(is.pointB.position)
          .scale(100)
          .sub(new Vector(50, 50))
      );
      const tangentVelocity = this.velocity.projectOn(
        is.pointA.position
          .copy()
          .sub(is.pointB.position)
          .scale(100)
      ...
      const springNormalVelocityAtImpact = springVelocityAtImpact
        .copy()
        .sub(springTangentVelocityAtImpact);
      const normalVelocity = this.velocity.copy().sub(tangentVelocity);

      tangentVelocity.add(
        springTangentVelocityAtImpact
          .copy()
          .sub(tangentVelocity)
          .scale(averageFriction)
      );
      springTangentVelocityAtImpact.add(
        tangentVelocity
          .copy()
          .sub(springTangentVelocityAtImpact)
          .scale(averageFriction)
      ...
      const finalNormalVelocity = springNormalVelocityAtImpact
        .copy()
        .scale(springMassAtImpact)
        .add(normalVelocity.copy().scale(this.mass))
        .scale(1 / (this.mass + springMassAtImpact))
        .scale(this.isFixed ? 0 : 1);

      const finalSpringVelocityAtImpact = finalNormalVelocity
        .copy()
        .add(springTangentVelocityAtImpact);

      const springVelocityDifferenceAtImpact = finalSpringVelocityAtImpact
        .copy()
        .sub(springVelocityAtImpact);

      if (!is.pointA.isFixed)
        is.pointA.velocity.add(
          springVelocityDifferenceAtImpact.copy().scale(momentB / totalMoment)
        );
      if (!is.pointB.isFixed)
        is.pointB.velocity.add(
          springVelocityDifferenceAtImpact.copy().scale(momentA / totalMoment)
        );
      ...
      if (this.isFixed) {
        ...
        if (!is.pointA.isFixed)
          is.pointA.position.add(
            positionDifference.copy().scale(momentB / totalMoment)
          );
        if (!is.pointB.isFixed)
          is.pointB.position.add(
            positionDifference.copy().scale(momentA / totalMoment)
          );
      } /* else {
  ...
  get neighbors(): Point[] {
    return (
      this._neighbors ||
      (this._neighbors = this.structure.springs
        .filter((s) => s.pointA == this || s.pointB == this)
        .map((s) => (s.pointA == this ? s.pointB : s.pointA)))
    );
  }
  neighborsSegments(position?: Vector, external = false) {
    return this.neighbors
      .filter((p) => (external ? p.isExternal : true))
      .map((p) => new Segment(p.position, position || this.position));
  }
  closestSpring(springs: Spring[], position?: Vector, isInside = false) {
    position = position || this.position;
    return springs
      .map((s) => {
        return { s, projection: s.segment.projection(position) };
      })
      .filter((x) => x.projection.t < 1.1 && x.projection.t > -0.1)
      .sort(
        (a, b) =>
          a.projection.projection.distanceSq(position) -
          b.projection.projection.distanceSq(position)
      )
      .map((d) => {
        return d.s;
      })[0];
  ...
  isInsideStructure(s: SoftStructure, position?: Vector) {
    let testSegment = new Segment(
      ...
      s.center.add(s.boundingBox.size)
    );
    return (
      s.springs.filter((sp) => sp.isSide && sp.segment.intersects(testSegment))
        .length %
        2 ==
      1
    );
  ...
  findInside(s: SoftStructure, found: Point[]) {
    let ns = this.neighbors.filter(
      (n) =>
        n.isExternal &&
        s.springs.every(
          (s) => !new Segment(this.position, n.position).intersects(s.segment)
        ) &&
        !found.find((n1) => n == n1) &&
        n.isInsideStructure(s)
    );
    found.push(...ns);
